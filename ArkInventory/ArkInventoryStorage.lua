-- perf: these are Blizzard client strings that never change during a session, so build the
-- soulbound/BoP search patterns once here instead of re-concatenating "^"..ITEM_X.."$" (and then
-- re-lowercasing them) on every single item scanned, in every bag, on every scan.
local ARKINV_PATTERN_SOULBOUND_ITEMS = {
	string.lower( "^" .. ITEM_SOULBOUND .. "$" ),
	string.lower( "^" .. ITEM_BIND_ON_PICKUP .. "$" ),
}

-- perf: shared function reused by every "sum up bag.count across all bags" call, instead of
-- each call site allocating its own fresh closure ( function( a ) return a.count end )
local function ARKINV_SumBagCount( a )
	return a.count
end
...
function ArkInventory.ScanBag( blizzard_id )
	...
	for slot_id = 1, bag.count do
		...
		if h then
			...
			-- perf: single pass over the tooltip lines checking both patterns, instead of
			-- two full separate scans (was walking every tooltip line twice per item)
			if ArkInventory.TooltipFindAny( ArkInventory.Global.Tooltip.Scan, ARKINV_PATTERN_SOULBOUND_ITEMS ) then
				sb = true
			end
	...
	if changed_bag then
		...
		cp.location[loc_id].slot_count = ArkInventory.Table.Sum( cp.location[loc_id].bag, ARKINV_SumBagCount )
...
function ArkInventory.FindDuplicates( )
	
	-- scans bag + bank for items occupying 2 or more separate slots (same item id,
	-- ignoring enchant/suffix variance) and stores the result for the fade/highlight code.
	-- deliberately NOT "total quantity >= 2" - that flags nearly every stackable consumable
	-- since people normally carry multiples of potions/food/etc, which floods the highlight
	-- and makes it useless. this instead flags redundant un-consolidated stacks.
	
	local cp = ArkInventory.Global.Me
	local seen = { }    -- itemId -> number of separate slots it occupies
	local dup = { }      -- itemId -> true, for items in 2+ slots
	
	for _, loc_id in pairs( { ArkInventory.Const.Location.Bag, ArkInventory.Const.Location.Bank } ) do
		
		local loc = cp.location[loc_id]
		if loc then
			for bag_id, bag in pairs( loc.bag ) do
				for slot_id, i in pairs( bag.slot ) do
					if i.h then
						local class, itemId = ArkInventory.ObjectStringDecode( i.h )
						if class == "item" and itemId then
							seen[itemId] = ( seen[itemId] or 0 ) + 1
							if seen[itemId] > 1 then
								dup[itemId] = true
							end
						end
					end
	...
	ArkInventory.Global.Duplicates = dup
	
end

function ArkInventory.ItemManualOrderGet( cat_id )
	
	if not ArkInventory.db.global.option.manual then
		ArkInventory.db.global.option.manual = { }
	end
	
	if not ArkInventory.db.global.option.manual[cat_id] then
		ArkInventory.db.global.option.manual[cat_id] = { }
	end
	
	return ArkInventory.db.global.option.manual[cat_id]
	
end

function ArkInventory.ItemManualMove( cat_id, itemId, direction )
	
	-- direction: -1 = move left/earlier, 1 = move right/later
	
	local order = ArkInventory.ItemManualOrderGet( cat_id )
	
	if #order == 0 then
		
		-- first time this category has been manually touched: seed the list so existing
		-- items keep their current relative order instead of jumping around
		local cp = ArkInventory.Global.Me
		local seedList = { }
		...
		for _, loc_id in pairs( { ArkInventory.Const.Location.Bag, ArkInventory.Const.Location.Bank } ) do
			...
			if loc then
				for bag_id, bag in pairs( loc.bag ) do
					for slot_id, i in pairs( bag.slot ) do
						if i.h then
							local c = i.cat or ArkInventory.ItemCategoryGet( i )
							if c == cat_id then
								local class, id = ArkInventory.ObjectStringDecode( i.h )
								if class == "item" and id and not seedList[id] then
									seedList[id] = true
	...
	if swapWith < 1 or swapWith > #order then
		return
	end
