...
function ArkInventory.TooltipFindAny( tooltip, patterns, IgnoreLeft, IgnoreRight, CaseSensitive )
	
	-- performance: same as calling TooltipContains/TooltipFind once per pattern, but only
	-- walks the tooltip lines ONCE regardless of how many patterns are being checked for.
	-- patterns must already be lower-cased by the caller if CaseSensitive is false/omitted,
	-- since we don't want to re-lowercase the same constant pattern on every single call.
			...
			if obj and obj:IsShown( ) then
				...
				if txt then
					...
					for p = 1, numPatterns do
						if string.find( txt, patterns[p] ) then
							return true
						end
					end
				end
...
local ARKINV_TOOLTIP_SIDE_NAMES = { "TextLeft", "TextRight" }

function ArkInventory.TooltipCanUse( tooltip )

	local l = ARKINV_TOOLTIP_SIDE_NAMES
