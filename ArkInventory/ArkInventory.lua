...
function ArkInventory.SortValueOrderGet( key, defaultList )
	
	if not ArkInventory.db.global.option.valueorder then
		ArkInventory.db.global.option.valueorder = { }
	end
	
	if not ArkInventory.db.global.option.valueorder[key] then
		local t = { }
		if defaultList then
		...
		ArkInventory.db.global.option.valueorder[key] = t
	end
	
	return ArkInventory.db.global.option.valueorder[key]
	
end

function ArkInventory.SortValueOrderRank( key, value, defaultList )
	
	-- returns this value's position in the ordered list, appending it to the end
	-- (and persisting that) the first time an unseen value shows up
	
	if value == nil then
		return 99999
	end
	
	local list = ArkInventory.SortValueOrderGet( key, defaultList )
...
function ArkInventory.SortValueOrderMove( key, value, direction, defaultList )
	...
	local swapWith = pos + direction
	if swapWith < 1 then
		swapWith = #list
	elseif swapWith > #list then
		swapWith = 1
	end
	
	list[pos], list[swapWith] = list[swapWith], list[pos]
...
function ArkInventory.ItemSortKeyGenerate( i, bar_id )
	...
	s["bagslot"] = s["!bagslot"] -- user-manageable alias; !bagslot below remains the guaranteed final tiebreak
	...
	if sorting.used and not sorting.bagslot then
		...
		-- item quality - uses your manually arranged order (Config -> Value Order),
		-- defaulting to the natural poor -> legendary/artifact ordering
		t = 0
		if i.q and sorting.active.quality then
			t = ArkInventory.SortValueOrderRank( "quality", i.q, { 0, 1, 2, 3, 4, 5, 6, 7 } )
		end
		s["quality"] = string.format( "%04i", t )
		
		
		-- location (equip slot) - uses your manually arranged order (Config -> Value Order),
		-- falling back to a sensible default (head, neck, shoulder... weapons... misc)
		t = "!"
		if i.h and class == "item" and sorting.active.location then
			...
			if not v10 or v10 == "" then
				...
				t = string.format( "%04i", ArkInventory.SortValueOrderRank( "location", v10, ArkInventory.Const.EquipLocationOrder ) )
			end
		...
		-- item type / subtype - uses your manually arranged order (Config -> Value Order).
		-- values are discovered automatically as you see items of that type; new ones
		-- default to the end of the list until you move them.
		local item_type = "!"
		local item_subtype = "!"
		...
		if i.h and class == "item" and sorting.active.itemtype then
			...
			if not item_type or item_type == "" then
				...
				item_type = string.format( "%04i", ArkInventory.SortValueOrderRank( "itemtype", item_type ) )
			end
			...
			if not item_subtype or item_subtype == "" then
				...
				item_subtype = string.format( "%04i", ArkInventory.SortValueOrderRank( "itemsubtype:" .. v7, item_subtype ) )
			end
		...
		if i.h and sorting.active.category then
			...
			if cat_type == ArkInventory.Const.Category.Type.Rule or cat_type == ArkInventory.Const.Category.Type.Custom then
				local cat = ArkInventory.db.global.option.category[cat_type].data[cat_code]
				if cat.used then
					cat_order = cat.order or 0
				end
			end
		...
		-- manual order (per category, user arranged via edit mode item menu)
		t = 99999999
		if i.h and sorting.active.manual then
			local mcat = i.cat or ArkInventory.ItemCategoryGet( i )
			local class, itemId = ArkInventory.ObjectStringDecode( i.h )
			if class == "item" and itemId and ArkInventory.db.global.option.manual and ArkInventory.db.global.option.manual[mcat] then
				for k, v in ipairs( ArkInventory.db.global.option.manual[mcat] ) do
					if v == itemId then
						t = k
						break
					end
		...
		s["manual"] = string.format( "%08i", t )
		
		
		-- per-criterion reverse: flips just this one field's contribution to the composite
		-- key, independent of the profile-wide ascending/descending toggle
		if sorting.reverse then
			for _, v in ipairs( sorting.order ) do
				if s[v] and sorting.reverse[v] then
					s[v] = ArkInventory.SortValueReverse( s[v] )
				end
			end
		end
...
function ArkInventory.SortKeyCheck( )
	...
	for sid, data in pairs( ArkInventory.db.global.option.sort.data ) do
		...
		if data.used then
			...
			-- check reverse table (new: per-criterion reverse, added alongside active)
			if not data.reverse or type( data.reverse ) ~= "table" then
				data.reverse = { }
				if data.reversed then
					-- migrate the old name-only reverse flag as a starting point
					data.reverse.name = true
				end
			end
			
			-- remove old keys from reverse table
			for k in pairs( data.reverse ) do
...
function ArkInventory.CategoryOrderMove( cat_type, id, direction )
	
	-- direction: -1 = move up/earlier, 1 = move down/later
	
	local v = ArkInventory.db.global.option.category[cat_type]
	
	local list = { }
	for k, cat in pairs( v.data ) do
		if cat.used then
			table.insert( list, { id = k, order = cat.order or 0 } )
		end
	end
	...
	-- normalize to sequential order values if they aren't already (eg never set before)
	for k, entry in ipairs( list ) do
		if entry.order ~= k then
			v.data[entry.id].order = k
			entry.order = k
		end
	end
	...
	local a, b = list[pos].id, list[swapWith].id
	v.data[a].order, v.data[b].order = v.data[b].order, v.data[a].order
...
function ArkInventory.SortValueReverse( s )

	-- inverts the lexicographic order of a formatted sort segment by complementing
	-- each byte. works for both text and zero-padded numeric segments since it's a
	-- monotonic, order-reversing transform regardless of content.
	...
	local t = { }
	for i = 1, #s do
		t[i] = string.char( 255 - string.byte( s, i ) )
	end
...
function ArkInventory.Frame_Main_OnHide( frame )
	...
	if ArkInventory.Global.ShowDuplicates then
		ArkInventory.Global.ShowDuplicates = false
	end
...
function ArkInventory.Frame_Item_Update_Fade( frame )
	...
	if ArkInventory.Global.ShowDuplicates then
		...
		if i and i.h then
			local class, itemId = ArkInventory.ObjectStringDecode( i.h )
			isDup = class == "item" and ArkInventory.Global.Duplicates and ArkInventory.Global.Duplicates[itemId]
		end
		if not isDup then
			fade = 0.2
		end
