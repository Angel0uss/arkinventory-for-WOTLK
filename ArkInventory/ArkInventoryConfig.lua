		...
		duplicates = {
			...
			args = {
				enabled = {
					...
					desc = "highlight items you have 2 or more separate stacks of, across bags and bank",
					type = "toggle",
					get = function( info )
						return ArkInventory.Global.ShowDuplicates
					end,
					set = function( info, v )
						ArkInventory.Global.ShowDuplicates = v
						if v then
							ArkInventory.FindDuplicates( )
						end
...
function ArkInventory.ConfigInternalCategoryCustom( path )
	...
	local args1 = {
		...
		up = {
			order = 300,
			name = "Move Up",
			desc = "move this category earlier in the display order",
			type = "execute",
			func = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.CategoryOrderMove( ArkInventory.Const.Category.Type.Custom, id, -1 )
				ArkInventory.ConfigInternalCategory( )
			end,
		},
		down = {
			order = 400,
			name = "Move Down",
			desc = "move this category later in the display order",
			type = "execute",
			func = function( info )
				local id = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.CategoryOrderMove( ArkInventory.Const.Category.Type.Custom, id, 1 )
...
function ArkInventory.ConfigInternalSortingCustom( path )
	...
	local args3 = {
		...
		rev = {
			...
			disabled = function( info )
				local id = ConfigGetNodeArg( info, #info - 3 )
				local key = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.option.sort.data[id].system or not ArkInventory.db.global.option.sort.data[id].active[key]
			end,
			name = ArkInventory.Localise["CONFIG_SORTMETHOD_REVERSE"],
			desc = ArkInventory.Localise["CONFIG_SORTMETHOD_REVERSE_TEXT"],
			type = "toggle",
			get = function( info )
				local id = ConfigGetNodeArg( info, #info - 3 )
				local key = ConfigGetNodeArg( info, #info - 1 )
				return ArkInventory.db.global.option.sort.data[id].reverse[key]
			end,
			set = function( info, v )
				local id = ConfigGetNodeArg( info, #info - 3 )
				local key = ConfigGetNodeArg( info, #info - 1 )
				ArkInventory.db.global.option.sort.data[id].reverse[key] = v
				ArkInventory.Frame_Main_Generate( nil, ArkInventory.Const.Window.Draw.Resort )
			end,
		},
		up = {
	...
	local function ValueOrderArgs( storeKey, defaultList, nameFunc )
		
		local list = ArkInventory.SortValueOrderGet( storeKey, defaultList )
		
		if not ArkInventory.Global.ConfigValueOrderShow then
			ArkInventory.Global.ConfigValueOrderShow = { }
		end
		...
		local args = {
			toggle = {
				...
				type = "toggle",
				name = "Show",
				desc = "show/hide the value order list",
				width = "half",
				get = function( )
					return ArkInventory.Global.ConfigValueOrderShow[storeKey]
				end,
				set = function( info, v )
					ArkInventory.Global.ConfigValueOrderShow[storeKey] = v
					ArkInventory.ConfigInternalSorting( )
				end,
			},
			togglespacer1 = {
		...
		local function isHidden( )
			return not ArkInventory.Global.ConfigValueOrderShow[storeKey]
		end
		...
		for k, value in ipairs( list ) do
			...
			args["label" .. k] = {
				order = 10 + ( k * 4 ),
				type = "execute",
				name = string.format( "%02i. %s", k, tostring( nameFunc( value ) or value ) ),
				width = "half",
				disabled = true,
				hidden = isHidden,
			}
			...
			args["up" .. k] = {
				...
				hidden = isHidden,
				func = function( )
					ArkInventory.SortValueOrderMove( storeKey, value, -1, defaultList )
					ArkInventory.ConfigInternalSorting( )
				end,
			}
	...
	for sortkey, active in pairs( ArkInventory.Const.SortKeys ) do
		...
		if active then
			...
			local thisArgs = args3
			if sortkey == "location" or sortkey == "quality" or sortkey == "itemtype" then
				
				-- deep-enough copy: args3's sub-tables must not be shared by reference across
				-- multiple tree positions, since AceConfigDialog keys some internal state off
				-- the option table's identity - reusing the same table object under three
				-- different criteria was corrupting path resolution for all three
				thisArgs = { }
				for k, v in pairs( args3 ) do
				...
				local defList, nameFn, criterionLabel
				if sortkey == "location" then
					defList = ArkInventory.Const.EquipLocationOrder
					nameFn = function( v ) return _G[v] end
					criterionLabel = "Equip Slot"
				elseif sortkey == "quality" then
					defList = { 0, 1, 2, 3, 4, 5, 6, 7 }
					nameFn = function( v ) return _G["ITEM_QUALITY" .. v .. "_DESC"] end
					criterionLabel = "Quality"
				else
					defList = nil
					nameFn = function( v ) return v end
					criterionLabel = "Item Type"
				end
				
				thisArgs.header = {
					order = 490,
					type = "header",
					name = "Value Order: " .. criterionLabel,
				}
				
				local vargs = ValueOrderArgs( sortkey, defList, nameFn )
				for k, v in pairs( vargs ) do
					v.order = 500 + ( v.order or 0 )
					thisArgs[k] = v
				end
