				...
				if level == 2 and value then
					...
					if value == "ACTIONS" then
						...
						ArkInventory.Lib.Dewdrop:AddLine(
							...
							"tooltipTitle", "Find Duplicates",
							"tooltipText", "highlight items you have 2 or more separate stacks of, across bags and bank",
							"closeWhenClicked", true,
							"func", function( )
								ArkInventory.Global.ShowDuplicates = not ArkInventory.Global.ShowDuplicates
				...
				if level == 1 then
					...
					if not isEmpty then
						...
						if class == "item" and itemId then
							ArkInventory.Lib.Dewdrop:AddLine(
								"text", "Move Left",
								"tooltipTitle", "Move Left",
								"tooltipText", "move this item one position earlier within its category (requires the manual order sort key enabled)",
								"closeWhenClicked", true,
								"func", function( )
									ArkInventory.ItemManualMove( cat0.id, itemId, -1 )
									ArkInventory.Frame_Main_Generate( nil, ArkInventory.Const.Window.Draw.Recalculate )
								end
							)
							ArkInventory.Lib.Dewdrop:AddLine(
								"text", "Move Right",
								"tooltipTitle", "Move Right",
								"tooltipText", "move this item one position later within its category (requires the manual order sort key enabled)",
								"closeWhenClicked", true,
								"func", function( )
									ArkInventory.ItemManualMove( cat0.id, itemId, 1 )
									ArkInventory.Frame_Main_Generate( nil, ArkInventory.Const.Window.Draw.Recalculate )
